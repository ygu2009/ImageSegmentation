...
fig=figure,
imshow(label2rgb(seg, mycolor)),title('MAPEM iter=1');
...
while iter<100 %& diff_Q>1e-10
    ...
    %     Q=sum(sum(abs(mu-mu_hat)./abs(mu)))+sum(sum(abs(sigma-sigma_hat)./abs(sigma))) +sum(abs(pipi-pipi_hat)./abs(pipi))
    %     diff_Q=Q/K;
